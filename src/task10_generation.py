...
def reorder_for_llm(chunks: list[dict]) -> list[dict]:
    ...
    # Xen kẽ: chunk hạng lẻ (0, 2, 4, ...) giữ nguyên thứ tự ở ĐẦU,
    # chunk hạng chẵn (1, 3, ...) đảo ngược đặt ở CUỐI.
    # → chunk tốt nhất (0) đứng đầu, chunk tốt nhì (1) đứng cuối,
    #   các chunk kém quan trọng hơn rơi vào giữa (nơi LLM dễ "quên" nhất).
    front = chunks[0::2]
    back = chunks[1::2]
    return front + back[::-1]
...
def generate_with_citation(query: str, top_k: int = TOP_K, use_reranking: bool = True) -> dict:
    ...
    # Step 3: Format context kèm source labels để LLM trích dẫn được
